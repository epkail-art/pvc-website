...
const properties: Array<{area: string; icon: React.ReactNode; title: string; description: string}> = [
  ...
  },
  {
    ...
    description: "Chlorine acts as a radical scavenger during combustion — PVC is self-extinguishing without additional flame retardant additives, unlike PE or PP.",
  },
  {
    area: "md:[grid-area:2/7/3/13] xl:[grid-area:1/8/2/13]",
    icon: <Thermometer className="h-4 w-4" />,
    title: "Tunable Stiffness",
    description: "Add plasticisers → flexible PVC. Remove them → rigid uPVC. Same base polymer, completely different material.",
  },
  {
    area: "md:[grid-area:3/1/4/13] xl:[grid-area:2/8/3/13]",
    icon: <Recycle className="h-4 w-4" />,
    title: "Durability & Recyclability",
    description: "PVC pipes last 40–100 years. VinylPlus recycled 771,000 tonnes in 2022 — resin code #3.",
  },
];
        ...
        <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl border-[0.75px] border-neutral-100 bg-white p-6 shadow-sm">
          <div className="relative flex flex-1 flex-col justify-between gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="font-sans text-sm leading-relaxed text-neutral-500">{description}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="what" className="py-32 max-w-6xl mx-auto px-6">
      ...
      >
        <p className="text-xs font-semibold tracking-[0.18em] uppercase text-teal-600 flex items-center gap-2 mb-4">
          <span className="w-5 h-px bg-teal-500 inline-block" />The Material
        </p>
